...
**Ce se întâmplă:** Keycloak validează credențialele și redirecționează utilizatorul înapoi către backend. Din perspectiva utilizatorului, nu vede nimic — e redirecționat direct spre pagina "completează-ți profilul".
...
3. **Dacă nu găsește nimic** (primul login al acestui utilizator): creează instant un rând "schelet" în `app_user` — doar `id_keycloak` și `mail` completate, `stareCont = INCOMPLET`, restul (`nume`, `prenume`, `facultate`, `rol`) rămân `NULL`. Redirecționează HTTP 302 către `{frontend}/complete-profile`.
4. **Dacă găsește** un rând existent, redirecționează spre pagina principală a aplicației — starea contului decide ce vede utilizatorul de acolo încolo (vezi tabelul din pasul 4).
...
[CustomAuthenticationSuccessHandler]
   │ id_keycloak nou? ──da──> creează app_user (stareCont=INCOMPLET) ──> redirect /complete-profile
   │
   └─nu (cont existent)──> redirect spre aplicație, StareContFilter decide ce vede
