...
1. Rețeaua `akadion_shared` trebuie să existe înainte de `docker-compose up`. Launcherul din akadion (`start-akadion.sh`, la rădăcina repo-ului) o creează automat când pornești stack-ul akadion. Dacă pornești embedder-ul separat/primul, creaz-o manual:
   ```powershell
